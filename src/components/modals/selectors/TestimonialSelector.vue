<template>
    <div class="selector_popup">
      <div class="input-area" @click="togglePopup(true)">
        <input type="text" v-model="searchQuery" placeholder="Search testimonials...">
...
const testimonials = computed(() => lookupStore.lookups.testimonials)
...
const filterTestimonials = () => {
    filtered.value = searchQuery.value.trim()
        ? testimonials.value.filter(t => t.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
        : testimonials.value
}
...
onMounted(async () => {
    ...
    if (!lookupStore.lookups.testimonials?.length) {
        await lookupStore.fetchTestimonials()
    }
})
  ...
  const getName = (id) => testimonials.value.find(t => t.id === id)?.name || 'Unknown'
